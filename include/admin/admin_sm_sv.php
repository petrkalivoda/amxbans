...
*/
	...
	//vraci servergrupy
	$query = mysql_query("SELECT group_id, name, is_default FROM ".$config->db_prefix."_servergroups ORDER BY is_default DESC, group_id") or die (mysql_error());
	$default_group_id = 0;
	$servergroups = array();
	while($row = mysql_fetch_object($query)) {
		$servergroups[] = (array)$row;
		if($row->is_default == 1) {
	...
	$smarty->assign("default_group_id", $default_group_id);
