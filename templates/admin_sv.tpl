								...
								<fieldset><legend>{"_SERVERSETTINGS"|lang} {$servers.hostname}</legend>
									...
									<form name="rcon_{$servers.sid}" method="POST">
										...
										<tr class="settings_line">
											<td>Skupina serverů:</td>
											<td>
												<select name="group_id">
													{foreach from=$servergroups item=group}
													<option value="{$group.group_id}"{if ($servers.group_id eq null && $group.group_id eq $default_group_id) || ($group.group_id eq $servers.group_id)} selected="selected"{/if}>{$group.name}</option>
													{/foreach}
												</select>
											</td>
										</tr>
